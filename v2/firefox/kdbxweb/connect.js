...
class KWPASS {
  init(callback) {
    ...
    this.file.open().then(callback);
  }
  async itl({url, submiturl}, callback) {
  ...
  async its(...args) {
    ...
    try {
      ...
      this.db.save().then(ab => this.attach(new Uint8Array(ab))).then(() => callback()).catch(e => callback(e.message));
    }
    catch (e) {
      callback('Is database unlocked? ' + e.message);
    }
  }
  async open(password) {
    const files = await this.file.read();
    if (files.length !== 1) {
      throw Error('number of internal files is ' + files.length + '. Go to options page and provide a database');
    }
